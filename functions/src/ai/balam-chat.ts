...
interface UserContext {
  ...
  locale?: string;
  briefMode?: boolean;
  // Persona selection (M1). When unset or unknown, defaults to "balam"
  // (the original Montessori parenting teacher). Premium gating for
...
function buildSystemPrompt(context: UserContext): string {
  const persona = getPersona(context.personaId);
  let prompt = persona.build({
    memberName: context.memberName,
    memberRole: context.memberRole,
    memberAgeMonths: context.memberAgeMonths,
    memberAgeYears: context.memberAgeYears,
    memberConditions: context.memberConditions,
    memberMedications: context.memberMedications,
  ...
  // Stage/role context is appended for the default Balam persona only.
  // Specialist personas embed their own age-aware norms in their identity
  // block (see personas/*.ts), so appending the Balam-flavored child stage
  // prompt would muddy their voice.
  if (persona.id === "balam") {
    if (isAdult) {
